...
List<SupportingResource> _supportingReferences(
  VaxPatient patient,
  Iterable<VaxSeries> series,
) {
  final refs = <SupportingResource>{};
  for (final s in series) {
    if (s.series.seriesType != SeriesType.risk) continue;
    var applicable = applicableIndications(
      patient,
      s.series,
      requireBeginAge: true,
    );
    if (applicable.isEmpty) {
      applicable = applicableIndications(
        patient,
        s.series,
        requireBeginAge: false,
      );
    }
    for (final ind in applicable) {
  ...
  final out =
      refs.toList()..sort(
        (a, b) => '${a.reference}|${a.display}'.compareTo(
          '${b.reference}|${b.display}',
        ),
      );
  return out;
}
...
List<VaxSeries> _determineBestPatientSeries(VaxAntigen antigen) {
  ...
  final prioritizedByGroup = <String, VaxSeries>{};
  ...
  final groupToEquiv = <String, EquivalentSeriesGroups>{};
  for (final entry in antigen.groups.entries) {
  ...
  final bestSeries = <VaxSeries>[];
  prioritizedByGroup.forEach((groupKey, pri) {
  ...
  for (final entry in prioritizedByGroup.entries) {
    ...
    final equivalentGroupKeys = <String>{};
    if (myEquiv != null) {
    ...
    var completeInEquivalent = false;
  ...
  ForecastTrace.current?.log(
    ...
    'best=${bestSeries.map((e) => e.series.seriesName).toList()}',
  );
  return bestSeries;
}
...
/// every preferable interval for that dose has an interval priority flag of
/// 'Y'.
bool _isPriorityForecast(VaxSeries series) {
  final td = series.targetDose;
  if (td >= (series.series.seriesDose?.length ?? 0)) return false;
  final seriesDose = series.series.seriesDose![td];
  final prefIntervals = seriesDose.preferableInterval;
  if (prefIntervals == null || prefIntervals.isEmpty) return false;
  return prefIntervals.every(
    (i) => i.intervalPriority == 'Y' || i.intervalPriority == 'override',
  );
}
...
_extractForecastVaccineInfo(VaxSeries series) {
  ...
  final doseNum = satisfiedCount + 1;
...
Map<String, List<VaccineGroupForecast>> _aggregateVaccineGroupForecasts(
  ...
  for (final entry in byGroup.entries) {
    ...
    if (!isMultiAntigen || antigens.length <= 1) {
      ...
      final bestList = _determineBestPatientSeries(antigen);
      if (bestList.isEmpty) {
        ...
        if (allPrioritized.isNotEmpty) {
          ...
          if (aggregated == SeriesStatus.agedOut ||
            ...
            (result[groupName] ??= <VaccineGroupForecast>[]).add(
              VaccineGroupForecast(
      ...
      if (riskBest.isNotEmpty && standardBest.isNotEmpty) {
        final uniqueValidDoseIds = <String>{};
      ...
      final partitions = <List<VaxSeries>>[];
      ...
      for (final part in partitions) {
        final partIsRisk = part.first.series.seriesType == SeriesType.risk;
        final partDoseNum = partIsRisk ? riskUnionDoseNum : null;
        ...
        if (part.length == 1) {
          ...
          (result[groupName] ??= <VaccineGroupForecast>[]).add(
            VaccineGroupForecast(
              ...
              supportingReferences: _supportingReferences(patient, <VaxSeries>[
                best,
              ]),
              antigensNeedingDose:
                  best.shouldRecieveAnotherDose
          ...
          final partSeriesNames = <String>{};
          final partSeriesGroupNames = <String>{};
          for (final s in part) {
            final n = s.series.seriesName;
            if (n != null) partSeriesNames.add(n);
            final g = s.series.selectSeries?.seriesGroupName;
          ...
          (result[groupName] ??= <VaccineGroupForecast>[]).add(
            VaccineGroupForecast(
              ...
              isRiskForecast: partIsRisk,
              antigensNeedingDose:
                  part.any((s) => s.shouldRecieveAnotherDose)
                      ? <String>[antigen.targetDisease]
                      : const <String>[],
    ...
    final bestByAntigen = <String, List<VaxSeries>>{};
    for (final antigen in antigens) {
      bestByAntigen[antigen.targetDisease] = _determineBestPatientSeries(
        antigen,
      );
    }
    ...
    bool typePresent({required bool risk}) => antigens.any(
      (a) => bestByAntigen[a.targetDisease]!.any(
        (s) => (s.series.seriesType == SeriesType.risk) == risk,
      ),
    );
    final passes = <bool>[
    ...
    for (final forRisk in passes) {
      final passBest = <String, List<VaxSeries>>{
        for (final VaxAntigen a in antigens)
          a.targetDisease:
              bestByAntigen[a.targetDisease]!
                  .where(
                    (s) => (s.series.seriesType == SeriesType.risk) == forRisk,
                  )
                  .toList(),
      };

      final statuses = <SeriesStatus>[];
      final earliestDates = <VaxDate>[];
      final recommendedDates = <VaxDate>[];
      final pastDueDates = <VaxDate>[];
      final latestDates = <VaxDate>[];
      final antigenNames = <String>[];
      final antigensNeedingDose = <String>[];
      ...
      final passSeriesNames = <String>{};
      final passSeriesGroupNames = <String>{};

      for (final antigen in antigens) {
        final bestList = passBest[antigen.targetDisease]!;
        if (bestList.isEmpty) continue;
        antigenNames.add(antigen.targetDisease);
        if (bestList.any((s) => s.shouldRecieveAnotherDose)) {
      ...
      ForecastTrace.current?.log(
        ...
        groupName,
        'antigens=${antigens.map((a) => a.targetDisease).toList()} '
      ...
      final doseNums = <int>[];
      for (final antigen in antigens) {
        final allBest = bestByAntigen[antigen.targetDisease]!;
        final bestList = passBest[antigen.targetDisease]!;
        if (bestList.isNotEmpty) {
          ...
          var antigenDoseNum = info.doseNum;
      ...
      if (doseNums.isNotEmpty) {
        ...
        final vgList = activeScheduleData.vaccineGroups?.vaccineGroup?.where(
          (g) => g.name == groupName,
        );
        final vgDef =
            (vgList != null && vgList.isNotEmpty) ? vgList.first : null;
        final useMin = vgDef?.administerFullVaccineGroup?.toString() == 'Yes';
        final aggregatedDoseNum =
            useMin
      ...
      (result[groupName] ??= <VaccineGroupForecast>[]).add(
        VaccineGroupForecast(
          ...
          seriesName:
              passSeriesNames.length == 1 ? passSeriesNames.first : null,
          seriesGroupName:
              passSeriesGroupNames.length == 1
          ...
          contributingSeries: passBest.values.expand((l) => l).toList(),
        ),
      );
    }
  }
...
ForecastResult evaluateForForecast(
  ...
  final agMap = antigenMap(patient);

  /// Build shared series group completion map and set up per-series state
  final seriesGroupCompletion = <String, Map<String, bool>>{};
  final seriesGroupCompletionDate = <String, Map<String, VaxDate>>{};
  agMap
    ..forEach((k, v) {
      seriesGroupCompletion[k] = <String, bool>{};
      seriesGroupCompletionDate[k] = <String, VaxDate>{};
      v.groups.forEach((key, group) {
        seriesGroupCompletion[k]![key] = false;
        for (final series in group.series) {
          series
            ..allPatientDoses = patient.pastDoses
            ..observations = patient.observations
            ..seriesGroupKey = key
            ..seriesGroupCompletion = seriesGroupCompletion
            ..seriesGroupCompletionDate = seriesGroupCompletionDate;
        }
      });
    })
    // Evaluate
    ..forEach((k, v) => v.evaluate())
    // Forecast
    ..forEach((k, v) => v.forecast());
...
Parameters forecastFromParameters(
  Parameters parameters, {
  ForecastMode mode = ForecastMode.cdc,
}) {
