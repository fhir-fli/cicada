...
Future<void> main() async {
      ...
      excelDir
          ...
          .where(
            (f) =>
                ...
                f.path.contains('AntigenSupportingData'),
          )
          .toList()
  ...
  for (final xlsx in files) {
    ...
    final antigen = name
        .replaceFirst('AntigenSupportingData- ', '')
        .replaceFirst('-508', '');
    String status;
    var diffs = <String>[];
    try {
      ...
      if (xmlModel.targetDisease == null &&
          (xmlModel.series?.isNotEmpty ?? false)) {
      ...
      final categories =
          (fromExcel.remove('vaccineRecommendationCategory') as List?)
              ?.length ??
          0;
    ...
    sink.writeln(
      '$antigen\t$status\t${diffs.length}\t${diffs.isEmpty ? '' : diffs.first}',
    );
    stdout.writeln(
      '$antigen: $status${diffs.isEmpty ? '' : ' (${diffs.length})'}',
    );
  ...
  await sink.close();
  stdout.writeln(
    '${files.length} workbooks, $failures not matching. ${out.path}',
  );
  exit(failures == 0 ? 0 : 1);
}
