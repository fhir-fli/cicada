...
9. The one exception is for recurring series (such as Flu or Tetanus). For these, anytime a target dose is met, a new target dose, identical to the original is created, and then compared against the remaining vaccines. This continues until there are no more doses to evaluate.
10. After evaluation, any doses that were not assigned a status (neither valid, not valid, nor sub-standard) are marked as **extraneous**. These are doses that didn't match any target dose in the series.

### Select Patient Series

We're going to look through all of the series we just evaluated and find the one (or occasionally more than one) that is the best recommendation to get the patient fully immunized. This involves pre-filtering, identifying prioritized series, and scoring when no single series is clearly the best. See the [Select Patient Series](06_select-patient-series.html) page for details.
